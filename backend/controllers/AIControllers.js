...
const generateAiNote = async (req, res) => {
  try {
    ...
    const response = await getAIResponse(userMessage, NOTE_GENERATION_RULES, "note");
...
const generateCanvasDrawing = async (req, res) => {
  try {
    ...
    const response = await getAIResponse(userMessage, CANVAS_GENERATION_RULES, "drawing");
